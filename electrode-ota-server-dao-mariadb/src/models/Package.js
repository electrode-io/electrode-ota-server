...
export default (sequelize) => {
    ...
    Package.hasMany(PackageDiff, {as: 'diffPackageMap', foreignKey: 'packageId', onDelete: 'CASCADE'});
    ...
    Package.prototype.createOrUpdateAssociate = function(updates) {
        const packDiff = _.find(this.diffPackageMap, {packageId: this.id_, packageHash: updates.packageHash});
        if (packDiff) {
            return packDiff.update(updates);
        } else {
            return PackageDiff.create(_.assign(updates, {packageId: this.id_, packageHash: updates.packageHash}))
                .then(packageDiff => {
    ...
    Package.prototype.toJSON = function() {
        ...
        asJson.diffPackageMap = _.keyBy(asJson.diffPackageMap,pkg => pkg.packageHash);
        return asJson;
    }
    return Package;
};
