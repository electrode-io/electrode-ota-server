...
const toSequelizeOptions = (modelDefinition, options) => {
    ...
    _.each(modelDefinition._associations, (association, assocationAs) => {
        extensions.include.push(toSequelizeIncludeTerm(assocationAs, association, null));
    });
    ...
    class ProxyModel {
        ...
        saveAsync(options={}) {
            if (this._getOriginal()) {
                ...
                return modelDefinition.create(asJson, extendedOptions)
        ...
        updateAsync(updates, options={}) {
            ...
            return client.transaction((transaction) => {
                return this._assignAssociations(jsonUpdates, { transaction })
                    .then(_ => this._getOriginal().update(jsonUpdates, Object.assign(seqOptions, {transaction})))
                    .then(model => this.refreshFromSequelizeModel(model));
            });
        }
        ...
        static _toSequelizeFormat(values) {
            ...
            if(modelDefinition._associations) {
                _.each(modelDefinition._associations, (assocation, associationAs) => {
                    ret[associationAs] = _.map(ret[associationAs],
                        (vals, key) => Object.assign({}, vals, {[assocation.searchField]: key})
                    );
                });
            }
            return ret;
        ...
        _assignAssociations(updates, options={}) {
            const actions = _.reduce(modelDefinition._associations,
                (accu, assoc, assocAs) => {
                    if (updates[assocAs]) {
                        accu = accu.concat(_.map(updates[assocAs], (assocItem) => {
                            if (this._getOriginal() && this._getOriginal().createOrUpdateAssociate) {
