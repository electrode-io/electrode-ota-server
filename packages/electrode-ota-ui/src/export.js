...
export default (data = _data, container = 'container', executor = fakeExecute)=> new Promise((resolve, reject)=> {
	...
	const app = ({isAuthenticated = false, isError = false, error})=> {
		...
		return resolve(render(<App store={createStore(data, executor)}/>, document.getElementById(container)));
	};
