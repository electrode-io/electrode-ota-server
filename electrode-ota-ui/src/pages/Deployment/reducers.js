...
import {DISMISS_ERROR} from '../Shell/actions';
import {generateReducer}from '../../util/reducerHelper';

export const selectedDeployment = (state = {}, {type, value, isError, error})=> {
	let isRefresh = false;
	switch (type) {
		case DISMISS_ERROR:
...
export const selectedDeploymentHistory = generateReducer(REQUEST_DEPLOYMENT_HISTORY, RECEIVE_DEPLOYMENT_HISTORY, 'items');
export const metrics = generateReducer(REQUEST_DEPLOYMENT_METRICS, RECEIVE_DEPLOYMENT_METRICS);
export default ({
	selectedDeployment,
