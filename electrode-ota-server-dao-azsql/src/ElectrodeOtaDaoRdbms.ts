...
export default class ElectrodeOtaDaoRdbms implements IElectrodeOtaDao {
    ...
    private testPool: ConnectionPool;

    public connect(options: any): Promise<void> {
        return new Promise((resolve, reject) => {
            if (this.testPool) {
                this.pool = this.testPool;
            } else {
                this.pool = new ConnectionPool(options.clusterConfig);
            }
    ...
    public async getMostRecentlyPromotedVersions(deploymentId: number): Promise<any> {
        return this.connectAndExecute<any>((conection) => {
            return PackageDAO.getMostRecentlyPromotedVersions(conection, deploymentId);
        });
    }
    // HISTORY functions ==================
    ...
    protected setPoolClusterForTest(c: ConnectionPool) {
        this.testPool = c;
    }
     ...
     */
    private async connectAndExecute<T>(daoMethod: (connection: ConnectionPool) => Promise<T>): Promise<T> {
        await this.getConnection();

        await this.setConnectionProperties(this.pool);

        return daoMethod(this.pool).then((ret) => {
            //connection.release();
            this.close();
            return ret;
        }).catch((err) => {
    ...
    private async setConnectionProperties(connection: ConnectionPool): Promise<void> {
